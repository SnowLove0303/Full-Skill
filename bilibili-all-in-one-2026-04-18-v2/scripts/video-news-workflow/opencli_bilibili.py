...
def _fetch_metadata_ytmdlp(bvid: str) -> Optional[dict]:
    ...
    try:
        ...
        cmd = f'yt-dlp {cookies_arg} --skip-download --print "%(upload_date)s|%(view_count)s|%(duration)s|%(title)s|{bvid}" "https://www.bilibili.com/video/{bvid}" 2>&1'
        ...
        for line in result.split('\n'):
            ...
            if '|' in line and bvid in line:
                ...
                if len(parts) >= 5:
                    ...
                    if pub_date and pub_date != 'NA' and len(pub_date) == 8:
                        pub_date_fmt = f"{pub_date[:4]}-{pub_date[4:6]}-{pub_date[6:8]}"
                    
                    return {
                        'bvid': bvid,
                        'title': title,
                        'pub_date': pub_date_fmt,
                        'title_date': title_date or pub_date_fmt,  # 如果标题没日期，用pub_date
                        'duration': duration,
                        'views': views,
